...
#[get("/posts/<id>")]
pub async fn get_post(db: &Db, _account: AuthorizedUser, id: Uuid) -> Result<Json<Post>, ServerError<GetPostError>> {
    let db_post = db::get_post(&mut db.0.begin().await.unwrap(), uuid::Uuid::from_bytes(*id.as_bytes()))
        .await?
        .ok_or(Expected(GetPostError::InvalidId("This id is invalid.".to_string())))?;
...
pub async fn get_posts(db: &Db, _account: AuthorizedUser, id: Uuid, limit: i64) -> Result<Json<Vec<Post>>, ServerError<GetPostsError>> {
    let db_posts: Vec<DbPost> = db::get_posts(&mut db.0.begin().await.unwrap(), uuid::Uuid::from_bytes(*id.as_bytes()), limit)
        ...
        .map_err(|err| {
            match err {
                DbError::RowNotFound => Expected(GetPostsError::InvalidId("This id is invalid.".to_string())),
                DbError::Unexpected(_) => Unexpected(format!("{:?}", err))
            }
        })?;

    Ok(Json(db_posts.into_iter().map(|db_post| Post {
        id: Uuid::from_bytes(*db_post.id.as_bytes()),
        message: db_post.message,
        posted_at: db_post.posted_at.date().as_ymd(),
        longitude: db_post.longitude,
...
#[get("/posts/map")]
pub async fn get_posts_map(db: &Db, _account: AuthorizedUser) -> Result<Json<Vec<Post>>, ServerError<()>> {
    let db_posts: Vec<DbPost> = db::get_newest_posts(&mut db.0.begin().await.unwrap())
        .await?;
